...
# restic_run_on.sh 10.0.0.1 <restic_bucket_from_values> restic_backup_mariadb.sh
# restic_run_on.sh 10.0.0.1 <restic_bucket_from_values> restic_backup_mariadb.sh 'MARIADB'
# restic_run_on.sh 10.0.0.1 <restic_bucket_from_values> restic_backup_mariadb.sh 'MARIADB --defaults-file "/etc/mysql/debian.cnf"'
# restic_run_on.sh 10.0.0.1 <restic_bucket_from_values> restic_backup_mariadb.sh 'MARIADB --defaults-file "/etc/mysql/debian.cnf" --ulimit-n 300000'
# restic_run_on.sh 10.0.0.1 <restic_bucket_from_values> restic_backup_mariadb.sh 'MARIADB --defaults-file "/etc/mysql/debian.cnf" --ulimit-n 300000 --add-innobackupex-option "--databases \"db1 db2\""'
# restic_run_on.sh 10.0.0.1 <restic_bucket_from_values> restic_backup_mariadb.sh 'MARIADB --defaults-file "/etc/mysql/debian.cnf" --ulimit-n 300000 --add-innobackupex-option "--databases \"db1 db2\"" --prune "--keep-hourly 3 --keep-within 30d"'
...
printf "%s\n" "${MARIABACKUP_COMMAND_LINE} | ${CREATE_COMMAND_LINE}"
bash -c "${MARIABACKUP_COMMAND_LINE}" | bash -c "${CREATE_COMMAND_LINE}"
