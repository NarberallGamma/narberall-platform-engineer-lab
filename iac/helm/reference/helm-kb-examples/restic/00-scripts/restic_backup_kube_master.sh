...
# restic_run_on.sh 10.0.0.1 <restic_bucket_from_values> restic_backup_kube_master.sh
...
then
  00-scripts/restic_backup_files.sh ETCD "${BACKUP_DIR}" --dont-ignore-missing-files --prune "${CUSTOMPRUNE}"
else
