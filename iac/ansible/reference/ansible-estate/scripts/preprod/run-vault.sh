...
check_ssl_certificates() {
    ...
    if [ ! -f "artifacts/preprod.example.com.crt" ] || [ ! -f "artifacts/preprod.example.com.key" ]; then
        echo "ERROR: SSL certificates not found in artifacts/ directory"
        echo ""
        echo "Required files:"
        echo "  - artifacts/preprod.example.com.crt"
        echo "  - artifacts/preprod.example.com.key"
        echo ""
        echo "See artifacts/README-ssl.md for instructions"
        exit 1
    fi
...
check_registry_credentials() {
    ...
    if grep -q "your_username" group_vars/preprod/vault_cluster.yml || grep -q "your_password" group_vars/preprod/vault_cluster.yml; then
        echo "ERROR: Docker registry credentials not configured in group_vars/preprod/vault_cluster.yml"
        echo ""
...
deploy_vault() {
    ...
    docker run --rm -it \
      ...
      ghcr.io/ansible/creator-ee:latest \
      ansible-playbook -i "$INVENTORY" playbooks/vault-deploy-preprod.yml "${@:2}"
...
init_vault() {
    ...
    docker run --rm -it \
      ...
      -e ANSIBLE_COLLECTIONS_PATHS=/work/.collections:/usr/share/ansible/collections \
      ghcr.io/ansible/creator-ee:latest \
      ansible-playbook -i "$INVENTORY" playbooks/vault-init-preprod.yml "${@:2}"
...
upload_secrets() {
    ...
    docker run --rm -it \
      ...
      ghcr.io/ansible/creator-ee:latest \
      ansible-playbook playbooks/vault-secrets-preprod.yml "${@:2}"
    
    if [ $? -eq 0 ]; then
        echo ""
        echo "SUCCESS: Secrets uploaded successfully!"
        echo ""
...
show_help() {
    ...
    echo "  - DNS Round Robin for high availability"
    echo ""
    echo "Architecture:"
    echo "  DNS: $VAULT_DOMAIN -> $VAULT_VIP"
    echo "  DNS -> 2 LB servers (Nginx + Keepalived)"
    ...
    echo "  secrets - Upload secrets to Vault (requires VAULT_TOKEN)"
    echo "  help    - Show this help"
    echo ""
    echo "Examples:"
    ...
    echo "  export VAULT_TOKEN=<token> && $0 secrets  # Upload secrets"
    echo ""
    echo "Prerequisites:"
    echo "  1. SSL certificates in artifacts/ directory"
    echo "  2. Docker registry credentials in group_vars/preprod/vault_cluster.yml"
