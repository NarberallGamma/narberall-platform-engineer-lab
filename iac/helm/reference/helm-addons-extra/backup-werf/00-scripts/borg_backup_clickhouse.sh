...
#   2. clickhouse-server must be running on the same node where
#      this script will run

# How it works:
#   - create local copies of table partitions with
#    'ALTER TABLE ... FREEZE PARTITION ...' in the 'shadow' subdirectory of
#     the clickhouse-server data directory (usually /var/lib/clickhouse)
...
# -h|--host                   - clickhouse-server connect addresses. Optional argument
# -r|--port                   - clickhouse-server connect port. Optional argument
# -u|--user                   - username used to connect to clickhouse-server
# -p|--password               - path to the user password file. Optional argument
# -t|--data-dir               - path to the clickhouse-server data directory; see
...
# -d|--db                     - target database name; may be given several times,
#                               all listed databases are backed up.
#                               Optional argument; if omitted,
#                               all databases are backed up
#    --only-supported-engines - when set, only tables whose engines are
#                               listed in ${SUPPORTED_ENGINES} enter the
#                               backup. Optional argument.
#                               Remember that with this option the backup may
#                               be incomplete, and no alert is sent about that
#    --debug                  - when set, print the list of tables and partitions
#                               that will be backed up. Optional argument
...
# the password itself. Its value must be a path to
# a password file. The file owner must be 'root:root' and
...
# Remove all occurrences of a string from the text
# ${1} - input text
# ${2} - match string
