...
#
# === Key with a passphrase (WSL / local run) ===
# Ansible in Docker is non-interactive — an encrypted key without ssh-agent will fail.
#
#   eval "$(ssh-agent -s)"
#   ssh-add ~/.ssh/your_key              # passphrase once per WSL session
...
# --ssh-agent only (no --ssh-key) — clears the key path and IdentitiesOnly from inventory; keys come from the agent.
#
