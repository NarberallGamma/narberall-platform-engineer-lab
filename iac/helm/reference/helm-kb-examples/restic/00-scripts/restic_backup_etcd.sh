...
do
  ...
  then
    ...
    ${KUBECTL} cp --retries=-1 "kube-system/${pod}:${ETCD_SNAPSHOT_FILE}" "${etcd_snapshot_backup_file}"
    kubectl_exit_status=${?}
    if test ${kubectl_exit_status} -ne 0;
    then
...
then
  00-scripts/restic_backup_files.sh ETCD "${BACKUP_DIR}" --dont-ignore-missing-files
else
