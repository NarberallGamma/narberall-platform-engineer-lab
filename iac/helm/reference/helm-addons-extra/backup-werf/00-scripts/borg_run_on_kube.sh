...
BACKUP_COMMAND_LINE="
  ...
  echo '### Fill known_hosts'
  ssh -A ${SSH_OPTIONS} ${PROXY_OPTION} '${BACKUP_USER}@${TARGET_HOST}' -p '${TARGET_PORT:-22}' 'ssh ${SSH_OPTIONS} \"${BORG_SERVER}\" \"borg -V\"'
  
  echo '### Run script'
  ssh -A ${SSH_OPTIONS} ${PROXY_OPTION} '${BACKUP_USER}@${TARGET_HOST}' -p '${TARGET_PORT:-22}' '${SCRIPT} ${SCRIPT_ARGS}'
"
