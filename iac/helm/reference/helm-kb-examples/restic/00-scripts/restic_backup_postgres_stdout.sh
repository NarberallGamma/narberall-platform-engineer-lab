...
#   - резервное копирование tar-архива с помощью restic с получением его из stdin
...
#    --skip-hostname-prefix      - позволяет исключить из restic репозитория поле хоста
#                                  в качестве hostname будет использоваться тег бэкапа
...
# restic_run_on.sh 10.0.0.1 <restic_bucket_from_values> restic_backup_postgres_stdout.sh
# restic_run_on.sh 10.0.0.1 <restic_bucket_from_values> restic_backup_postgres_stdout.sh 'PG'
# restic_run_on.sh 10.0.0.1 <restic_bucket_from_values> restic_backup_postgres_stdout.sh 'PG --user postgres'
# restic_run_on.sh 10.0.0.1 <restic_bucket_from_values> restic_backup_postgres_stdout.sh 'PG --user postgres --do-su-under-user'
# restic_run_on.sh 10.0.0.1 <restic_bucket_from_values> restic_backup_postgres_stdout.sh 'PG --user postgres --do-su-under-user -a "--max-rate 1024" -a --progress'
# restic_run_on.sh 10.0.0.1 <restic_bucket_from_values> restic_backup_postgres_stdout.sh 'PG --user postgres --do-su-under-user -a "--max-rate 1024" -a --progress --prune "--keep-hourly 3 --keep-within 30d"'
# restic_run_on.sh 10.0.0.1 <restic_bucket_from_values> restic_backup_postgres_stdout.sh 'PG --host 127.0.0.1 --user pg_super --password "/etc/backup/pg-pass"'
...
CUSTOMPRUNE=""
DO_SU_UNDER_USER=""
DO_NOT_USE_HOSTNAME_IN_RESTIC_REPO_NAME=""
...
NORMALIZED_ARGS="$( getopt --options h:r:u:p:a:k: --longoptions ,host:,port:,user:,password:,add-pg_basebackup-option:,prune:,do-su-under-user,skip-hostname-prefix -- "${@}" 2>$ERRLOG )"
if test "${?}" -ne 0;
then
...
then
  ...
  then
    printf "%s\n" "su - \"${USER}\" -c \"${PG_BASEBACKUP_COMMAND_LINE}\" | ${CREATE_COMMAND_LINE}"
    su - "${USER}" -c "${PG_BASEBACKUP_COMMAND_LINE}" | bash -c "${CREATE_COMMAND_LINE}"
  else
    printf "%s\n" "${PG_BASEBACKUP_COMMAND_LINE} | ${CREATE_COMMAND_LINE}"
    bash -c "${PG_BASEBACKUP_COMMAND_LINE}" | bash -c "${CREATE_COMMAND_LINE}"
  fi
  ...
  then
    alert "restic create failed, exit code ${CREATE_EXIT[1]}. Pruning of old archives skipped" "`cat $ERRLOG`"
    rm $ERRLOG
    exit 1
  fi
else
  ...
  then
    printf "%s\n" "su - \"${USER}\" -c \"${PG_BASEBACKUP_COMMAND_LINE}\" | ${CREATE_COMMAND_LINE}"
    printf "%s" "${PASSWORD_EVOLVED}" | su - "${USER}" -c "${PG_BASEBACKUP_COMMAND_LINE}" | bash -c "${CREATE_COMMAND_LINE}"
  else
    printf "%s\n" "${PG_BASEBACKUP_COMMAND_LINE} | ${CREATE_COMMAND_LINE}"
    printf "%s" "${PASSWORD_EVOLVED}" | bash -c "${PG_BASEBACKUP_COMMAND_LINE}" | bash -c "${CREATE_COMMAND_LINE}"
  fi
  ...
  then
    alert "restic create failed, exit code ${CREATE_EXIT[2]}. Pruning of old archives skipped" "`cat $ERRLOG`"
    rm $ERRLOG
    exit 1
  fi
fi
...
then
  alert "restic prune failed, exit code ${PRUNE_EXIT}" "`cat $ERRLOG`"
  rm $ERRLOG
  exit 1
fi
