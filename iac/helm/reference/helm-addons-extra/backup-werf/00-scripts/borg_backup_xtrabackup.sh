...
# How it works:
#   - invoke xtrabackup and stream the dump to stdout
...
#                                 socket, and similar (xtrabackup --defaults-file).
...
# -a|--add-innobackupex-option  - extra option passed to xtrabackup. If the
#                                 xtrabackup option has a value, pass it with
...
#                                 listed options are passed to xtrabackup.
