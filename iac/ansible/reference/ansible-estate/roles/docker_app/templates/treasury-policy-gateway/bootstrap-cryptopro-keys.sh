...
if [ "$(echo "${KEYS_IMPORT_ENABLED}" | tr '[:upper:]' '[:lower:]')" = "true" ]; then
  if [ "$(echo "${KEYS_IMPORT_FORCE}" | tr '[:upper:]' '[:lower:]')" = "true" ]; then
    ...
    echo "Keys already present, skipping import"
  else
...
if keys_exist; then
  echo "Keys store populated: skip recursive chown/chmod (~100k dgtry_mk_* on VM)"
  chown "${USER_ID}:${GROUP_ID}" "${KEYS_DIR}" 2>/dev/null || true
...
if [ "${MODE_ENV}" = "PREGEN" ]; then
  ...
  elif [ "$(echo "${KEYS_IMPORT_ENABLED}" | tr '[:upper:]' '[:lower:]')" != "true" ]; then
    echo "PREGEN: keys dir empty, import disabled — skip auto-generation (manual migration from hsm-adapter expected)"
...
if keys_exist; then
  echo "Skip recursive chown/chmod on ${TARGET} (keys already present)"
else
...
ls -ld "${TARGET}" "${KEYS_DIR}" 2>/dev/null || ls -ld "${TARGET}"
if keys_exist; then
  sample=$(find "${KEYS_DIR}" -maxdepth 1 -type d -name 'dgtry_mk_*' -print -quit 2>/dev/null || true)
  echo "Keys sample: ${sample:-none}"
else
