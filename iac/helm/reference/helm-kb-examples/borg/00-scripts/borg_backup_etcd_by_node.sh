...
IFS=$'\n'
ETCD_NODE=${1:-non-specified}

if [ $(ip a | grep $(${KUBECTL} describe no ${ETCD_NODE} |grep InternalIP | awk -F: '{print $2}'|xargs) | wc -l) -ne 1 ] ; then
  alert "IP of ${ETCD_NODE} node does not match this machine"
  exit 1
fi
...
if test -z "${PODS}";
then
  alert "Could not find etcd pods on ${ETCD_NODE} node"
  exit 1
fi
...
do
  ...
  etcd_ca_cert_current_pod="/etc/kubernetes/pki/etcd/ca.crt"
  #etcd_ca_cert_current_pod="$( get_etcdctl_liveness_ca_cert "${pod_liveness}" )"

  etcd_client_cert_current_pod="/etc/kubernetes/pki/etcd/healthcheck-client.crt"
  #etcd_client_cert_current_pod="$( get_etcdctl_liveness_client_cert "${pod_liveness}" )"

  etcd_client_key_current_pod="/etc/kubernetes/pki/etcd/healthcheck-client.key"
  ...
  then
    ...
    ${KUBECTL} -n kube-system exec "${pod}" -- sh -c "ETCDCTL_API=3 etcdctl ${tls_string} snapshot save ${ETCD_SNAPSHOT_FILE}"
    kubectl_exit_status=${?}
    if test ${kubectl_exit_status} -ne 0;
    then
      alert "etcdctl snapshot save for pod ${pod} failed: ${kubectl_exit_status}"
      let errors_count+=1
      continue
    fi
...
#
#    ${KUBECTL} cp  "kube-system/${pod}:${ETCD_SNAPSHOT_FILE}" "${etcd_snapshot_backup_file}"
#    kubectl_exit_status=${?}
#    if test ${kubectl_exit_status} -ne 0;
#    then
#      alert "etcdctl snapshot copy for pod ${pod} failed: ${kubectl_exit_status}"
#      let errors_count+=1
#      continue
#    fi
  fi
  ...
  then
    ...
    ${KUBECTL} -n kube-system exec "${pod}" -- sh -c "etcdctl version" >> "${etcd_version_backup_file}"

#    ${KUBECTL} cp "kube-system/${pod}:${etcd_data_dir_current}" "${etcd_pod_backup_dir}"
#    kubectl_exit_status=${?}
#    if test ${kubectl_exit_status} -ne 0;
#    then
...
then
  00-scripts/borg_backup_files.sh ETCD "${BACKUP_DIR}" --dont-ignore-missing-files
else
