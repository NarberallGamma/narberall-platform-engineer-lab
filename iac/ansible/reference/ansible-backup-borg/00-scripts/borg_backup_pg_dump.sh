...
# Restoring data from a backup produced by pg_dump
# takes significantly longer than with pg_basebackup and
# on large datasets (~300 GB) may take a full day
...
#   - otherwise:
#     - create and store database dumps in temporary directory ${TMP_DIR} with pg_dump
...
#                            databases are included. If several databases
#                            are given, each backup is stored in a separate
#                            Borg repository whose name includes the job name
#                            and the database name, i.e. $(hostname)-${db}-${NAMEOFBACKUP}
#                            is used instead of $(hostname)-${NAMEOFBACKUP}.
#                            All database backups may be placed in one archive
#                            with --one-archive. At least one database
#                            or --all-db is required
#    --all-db              - back up all databases served by the current
#                            PostgreSQL instance. The database list is taken
#                            from PostgreSQL and then the same algorithm is
#                            used as when the list is built manually
#                            with -d|--db, i.e. each database gets a separate
#                            Borg repository. See
#                            -d|--db and --one-archive
# -e|--exclude-db          - database name to exclude from the backup;
#                            may be given more than once; all listed
#                            databases are excluded
# -a|--add-pg_dump-option  - extra option passed to
#                            pg_dump. If the pg_dump option has a
#                            value, pass it either with an equals sign
#                            ( = ) (long options only) or with a space,
#                            in which case the pg_dump option and its
#                            value must be wrapped in double or
#                            single quotes. For example:
#                             - --add-pg_dump-option --jobs=4
#                             - --add-pg_dump-option '--jobs 4'
#                             - --add-pg_dump-option "--jobs 4"
#                            May be given more than once; all listed options
#                            are passed to pg_dump. Optional
# -k|--prune               - retention options in Borg format, for
#                            example '--keep-hourly 72 --keep-within=30d'
#                            Optional; if omitted,
#                            ${CUSTOMPRUNE_DEFAULT} is used
#    --do-su-under-user    - run pg_dump as the user given by
#                            -u|--user or the default user. Useful when
#                            the 'peer' authentication method in pg_hba.conf
#                            must be used instead of 'trust'
#    --one-archive         - store backups of different databases in one
#                            archive. Database backups are written to a
#                            temporary directory. The filesystem must have
#                            enough free space to hold them.
#                            If the list contains only one database, the
#                            temporary directory is not used
#    --tmp-dir             - temporary directory used to store
#                            database backups. Optional; if omitted,
