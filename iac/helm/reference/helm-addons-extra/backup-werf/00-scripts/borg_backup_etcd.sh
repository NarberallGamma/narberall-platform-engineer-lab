...
# This script is the primary way to back up the control plane.
# It backs up the etcd cluster and the contents of /etc/kubernetes.

# The script must run on a node with Kubernetes Master components

# How it works:
#   - connect to the etcd cluster with etcdctl from the master node
