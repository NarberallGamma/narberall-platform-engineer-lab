#!/usr/bin/env bash
# Run xui_docker via ansible-runner. Execute from iac/ansible/reference/ansible-edge/.
set -euo pipefail
...
ensure_ansible_image() {
  ...
  case "$IMAGE_PULL_POLICY" in
    pull|auto)
      ...
      ;;
    local)
      echo "run_edge.sh: image $ANSIBLE_IMAGE not found. Build: ../ansible-runner/" >&2
      exit 1
      ;;
  esac
}
