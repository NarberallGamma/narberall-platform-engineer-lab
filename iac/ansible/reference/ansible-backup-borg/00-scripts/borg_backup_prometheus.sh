...
# Primary way to back up Prometheus

# Applicable when all of the following hold:
#   1. Prometheus version >= 2.1
#   2. Prometheus is running on the same node where
#      this script will run
#   3. Prometheus is started with --web.enable-admin-api

# How it works:
#   - create a snapshot via '/api/v1/admin/tsdb/snapshot' in the 'snapshots'
#     subdirectory of the Prometheus data directory (usually /var/prometheus/data/)
...
#                               if omitted, ${NAMEOFBACKUP_DEFAULT} is used
# -h|--host                   - Prometheus connection address. Optional;
#                               if omitted, ${HOST_DEFAULT} is used
# -r|--port                   - Prometheus connection port. Optional;
#                               if omitted, ${PORT_DEFAULT} is used
# -u|--user                   - username used to connect
#                               to Prometheus. Optional
# -p|--password               - path to the password file used to
#                               connect to Prometheus, or the name of an
#                               environment variable that holds that password. Optional
# -t|--data-dir               - Prometheus data directory. Optional;
#                               if omitted, ${DATA_DIR_DEFAULT} is used
# -l|--location               - location in the HTTP request used to create the snapshot. Optional;
#                               if omitted, ${LOCATION_DEFAULT} is used
